...
from config.paths import (
    LABELED_RUNS_CSV, MODEL_DIR, CLASSIFIER_PATH, FEATURE_SCHEMA_PATH,
)
from features.feature_extractor import FEATURE_NAMES


def load_data(labeled_data_path: str = LABELED_RUNS_CSV):
...
def train_decision_tree(X: np.ndarray, y: np.ndarray, cv_folds: int = 5):
...
def train_xgboost(X: np.ndarray, y: np.ndarray, cv_folds: int = 5):
...
def train(
    labeled_data_path: str = LABELED_RUNS_CSV,
    ...
    X, y, df = load_data(labeled_data_path)
    print(f"Loaded {len(df)} samples ({y.sum()} GRAPH, {len(y) - y.sum()} SQL)")

    # Train both models
    dt_model, dt_cv = train_decision_tree(X, y, cv_folds)
    print(f"Decision Tree CV F1: {dt_cv['cv_f1_mean']:.3f} +/- {dt_cv['cv_f1_std']:.3f}")

    xgb_model, xgb_cv = train_xgboost(X, y, cv_folds)
    ...
    # Full evaluation
    dt_eval = evaluate_model(dt_model, X, y, "DecisionTree")
    xgb_eval = evaluate_model(xgb_model, X, y, "XGBoost")
