...
\author{
  \IEEEauthorblockN{Priyanshu}
  \IEEEauthorblockA{
    \textit{Big Data Analytics Research}\\
    \textit{hifun\_router --- SIGMOD 2026 Submission}
  }
}
...
Phase~2 provisions a production-grade nine-service Hadoop~+~Spark cluster via Docker Compose. The services and their roles are summarized in Table~\ref{tab:cluster}.

\begin{table}[h]
\centering
\caption{Docker Compose Cluster Services}
\label{tab:cluster}
\begin{tabular}{llp{3.8cm}}
\toprule
\textbf{Service} & \textbf{Image} & \textbf{Role} \\
\midrule
\texttt{namenode}         & hadoop-namenode:3.2.1-java8  & HDFS metadata + block location index \\
\texttt{datanode}         & hadoop-datanode:3.2.1-java8  & HDFS data block storage (128~MB blocks) \\
\texttt{resourcemanager}  & hadoop-resourcemanager:3.2.1 & YARN app scheduling \& resource allocation \\
\texttt{nodemanager}      & hadoop-nodemanager:3.2.1     & YARN container execution on worker node \\
\texttt{spark-master}     & Spark~3.4.2 standalone       & Spark standalone master (port 7077) \\
\texttt{spark-worker-1}   & Spark~3.4.2 standalone       & Worker: 4~GB RAM, 2 cores \\
\texttt{spark-worker-2}   & Spark~3.4.2 standalone       & Worker: 4~GB RAM, 2 cores \\
\texttt{spark-history}    & Spark~3.4.2 history server   & Job history UI at port 18080 \\
\texttt{jupyter-lab}      & JupyterLab + PySpark kernel  & Interactive analysis at port 8888 \\
\bottomrule
\end{tabular}
\end{table}

All services share the \texttt{hifun-net} Docker bridge network (172.28.0.0/16). Parquet data and Spark event logs are mounted from named Docker volumes for persistence across container restarts. The GraphFrames JAR (\texttt{graphframes:graphframes:0.8.3-spark3.4-s\_2.12}) is specified via \texttt{SPARK\_PACKAGES} and downloaded at container start. The cluster is started with \texttt{docker-compose up -d} and is the target environment for the real-engine label collection script described in Section~\ref{sec:phase2_labels}.
...
\begin{table}[h]
\centering
\caption{Routing Accuracy Comparison (5-Fold CV for ML Models; 341 labeled subexpressions --- 231 SQL, 110 GRAPH)}
\label{tab:routing}
\begin{tabular}{lcccc}
\toprule
\textbf{Strategy} & \textbf{Acc.} & \textbf{Macro-F1} & \textbf{Prec.} & \textbf{Recall} \\
\midrule
Always SQL          & 0.677 & 0.000 & 0.000 & 0.000 \\
Always Graph        & 0.323 & 0.488 & 0.323 & 1.000 \\
Threshold Rule      & 0.836 & 0.659 & 1.000 & 0.491 \\
Logistic Regression & 1.000 & 1.000 & 1.000 & 1.000 \\
\textbf{Decision Tree (ML)} & \textbf{1.000} & \textbf{1.000} & \textbf{1.000} & \textbf{1.000} \\
Trivial Rule        & 1.000 & 1.000 & 1.000 & 1.000 \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textit{Discussion.} Single-engine strategies bound performance: Always SQL scores 67.7\% accuracy and fails to identify any GRAPH-optimal subexpression (F1~$=0$ for GRAPH class); Always Graph scores 32.3\% accuracy and correctly recalls all 110 GRAPH subexpressions but misclassifies all 231 SQL subexpressions. The Threshold Rule, which conditions GRAPH routing on \texttt{avg\_degree}~$> 5.0$, \texttt{max\_hops}~$\leq 3$, and \texttt{selectivity}~$\geq 0.01$, achieves 83.6\% accuracy (F1~$=0.659$) but recalls only 49.1\% of GRAPH samples because the selectivity and degree constraints are too strict on the current dataset distribution. Logistic Regression on the same 22-dimensional feature vector matches the Decision Tree at 100\% accuracy, confirming that the decision boundary is linearly separable under the current heuristic cost-model labels. The Trivial Rule (TRAVERSAL~$\to$~GRAPH, else~$\to$~SQL) and both ML classifiers all achieve perfect routing, reflecting the dominance of the \texttt{has\_traversal} feature in cost-model-derived labels.

Although Trivial Rule and ML both reach 100\% accuracy on simulation labels, the ML framework provides two advantages: (1)~SHAP-based feature importance and ablation analyses identify which features contribute to routing decisions, providing actionable guidance for cost model refinement and real-engine label collection; (2)~the 22-dimensional feature space positions the classifier to learn nuanced decision surfaces when real-engine labels---where the SQL/GRAPH tradeoff depends on cardinality, selectivity, and degree distribution---are collected via the Phase~2 measurement infrastructure.
...
\subsection{Cross-Dataset Generalization (Phase~2)}

The cross-dataset generalization experiment (Section~\ref{sec:phase2_cross}) tests whether the XGBoost classifier trained on one dataset's subexpression distribution generalizes to another. The experiment is implemented and the infrastructure is validated; full quantitative results require the real-engine labeled dataset ($\geq$500 samples) produced by the Phase~2 measurement script. With the current 186 simulation-labeled samples, the dataset split into three groups (TPC-H, SNB, Synthetic) yields insufficient per-group sample counts for statistically robust cross-dataset evaluation. Table~\ref{tab:crossds} provides the expected output schema.

\begin{table}[h]
\centering
\caption{Cross-Dataset Generalization Report Schema}
\label{tab:crossds}
\begin{tabular}{llccc}
\toprule
\textbf{Train} & \textbf{Test} & \textbf{$N_{\text{train}}$} & \textbf{F1 (base)} & \textbf{F1 (+20\% FT)} \\
\midrule
tpch     & snb       & -- & -- & -- \\
tpch     & synthetic & -- & -- & -- \\
snb      & tpch      & -- & -- & -- \\
...
\noindent Entries to be populated with real-engine label collection results. The heatmap PDF generated by \texttt{cross\_dataset\_generalization.py} will replace Table~\ref{tab:crossds} upon completion.

\subsection{Scale Factor Latency Experiment (Phase~2)}

The scale factor experiment (Section~\ref{sec:phase2_sf}) evaluates routing strategies at TPC-H SF=1 and SF=5. TPC-H~SF=5 Parquet data generation is scripted in \texttt{data/scripts/generate\_tpch\_sf5.sh}. The experiment infrastructure is fully implemented; results from distributed Spark runs are pending cluster execution. Table~\ref{tab:sflatency} provides the expected output schema.

\begin{table}[h]
\centering
\caption{Scale Factor Latency Experiment Schema}
\label{tab:sflatency}
\begin{tabular}{llcc}
\toprule
\textbf{Scale Factor} & \textbf{Strategy} & \textbf{Median (ms)} & \textbf{p95 (ms)} \\
\midrule
SF=1 & \texttt{always\_sql}  & -- & -- \\
SF=1 & \texttt{threshold}    & -- & -- \\
SF=1 & \texttt{learned\_xgb} & -- & -- \\
SF=5 & \texttt{always\_sql}  & -- & -- \\
SF=5 & \texttt{threshold}    & -- & -- \\
SF=5 & \texttt{learned\_xgb} & -- & -- \\
\bottomrule
\end{tabular}
\end{table}

\noindent Entries to be populated with real Spark cluster execution results from \texttt{scale\_factor\_results.csv}.
...
Phase~2 significantly expands the system's infrastructure readiness for production measurement. Five components were fully implemented: (1)~\texttt{SparkSQLGenerator} and \texttt{GraphFramesGenerator} that execute SubExpressions on real PySpark~3.4.2 with Catalyst optimization and actual GraphFrames~0.8.3 BFS, replacing the pandas simulation; (2)~a nine-service Docker Compose cluster (HDFS, YARN, Spark standalone with two workers, History Server, JupyterLab) that provides the distributed substrate; (3)~a real label collection script that times each SubExpression on both engines with 2~warm-up and 3~measured runs, targeting $\geq$500 samples in \texttt{real\_labeled\_runs.csv} with a \texttt{label\_source=``real\_measurement''} provenance field; (4)~a query variant generator sweeping selectivity, traversal depth, and join type to augment training data beyond the 186-sample simulation baseline; and (5)~cross-dataset generalization and SF=1 vs.\ SF=5 scale factor experiment scripts with validated infrastructure and report schemas ready for cluster execution.

Routine accuracy comparison across 341 labeled subexpressions (231~SQL, 110~GRAPH) confirms: Always~SQL reaches 67.7\% accuracy, failing all GRAPH-optimal subexpressions; The Threshold Rule reaches 83.6\% (F1~$=0.659$) by over-constraining degree and selectivity conditions; Logistic Regression and Decision Tree both achieve 100\% accuracy, confirming linear separability under the current cost-model labels. SHAP-based interpretability analysis identifies traversal presence, BFS depth, and graph connectivity as dominant discriminative features. VIF analysis with exact scores (\texttt{max\_degree}~304.22, \texttt{avg\_degree}~303.04, \texttt{degree\_skew}~242.44) quantifies the collinearity structure and motivates feature consolidation for real-engine label regimes where all 22 dimensions contribute independently.
