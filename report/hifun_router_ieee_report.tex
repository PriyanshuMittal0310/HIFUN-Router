...
\texttt{experiments/results/relevance\_eval\_strict.json}.

\begin{table}[h]
\centering
\caption{Strict Real-Data Headline Metrics (Held-Out Eval)}
...
Robustness diagnostics from \texttt{experiments/results/strict\_robustness\_eval.json} report a bootstrap 95\% interval for XGBoost F1 of [0.939, 1.000] (median 0.983), and cross-dataset transfer F1 of 0.974 (OGB$\rightarrow$SNB) and 0.980 (SNB$\rightarrow$OGB), indicating strong but still small-sample evidence.

\subsection{Legacy Simulation Snapshot (Archived Context)}

The following subsection reports earlier simulation-labeled results and is retained only as historical development context. These numbers are not used for headline claims.
...
This paper presented HIFUN Router, an ML-based system for automatic query decomposition and intelligent engine routing in hybrid SQL/Graph workloads. The current submission snapshot prioritizes strict real-measurement evidence: a curated non-degenerate dataset of 288 labeled SubExpressions (151 SQL, 137 GRAPH), deterministic strict train/eval splits (231/57), and held-out strict evaluation where XGBoost, Decision Tree, and Logistic Regression each achieve F1\,$\approx$\,0.982 while Always-SQL collapses to F1\,$=0$. Bootstrap analysis gives a 95\% interval of [0.939, 1.000] for XGBoost F1, and cross-dataset transfer between OGB and SNB remains high (F1\,$\approx$\,0.974--0.980), providing stronger evidence than simulation-only analysis.

The end-to-end pipeline---spanning DSL parsing, dependency-aware decomposition, feature extraction, ML prediction, dual-engine execution, and cross-engine result composition---continues to be validated by checksum-based correctness testing. Remaining failures are concentrated in multi-hop BFS schema normalization for selected SNB/synthetic cases, which are implementation issues rather than classifier validity issues.
...
Legacy simulation-labeled results (for 186/341-sample development-era datasets) remain useful as engineering history and feature-design motivation, but are explicitly treated as archival context rather than the primary empirical claim.
